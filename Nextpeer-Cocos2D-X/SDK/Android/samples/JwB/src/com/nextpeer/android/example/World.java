...
import com.nextpeer.android.example.P2PMessage.P2PMessagePlayerHeight;
...
public class World {
    ...
    public volatile Map<String, OpponentHeightSoFar> playerIdToHeightSoFar; // Key(PlayerId) | Value(OpponentHeightSoFar) - Can be accessed by the render 
    ...
    public World(WorldListener listener, long seed) {
        this.bob = new Bob(5, 1);
        this.platforms = new ArrayList<Platform>();
        this.springs = new ArrayList<Spring>();
        this.squirrels = new ArrayList<Squirrel>();
        this.coins = new ArrayList<Coin>();
        this.playerIdToHeightSoFar = new ConcurrentHashMap<String, OpponentHeightSoFar>();
        
        this.listener = listener;
        
        // Seeding the random method so it will create equal level for all players
...
public void update(float deltaTime, float accelX) {
    ...
    if (atTheBottomOfScreen()) {

        if (listener.shouldEndGame()) {
            state = WORLD_STATE_GAME_OVER;
        }
        else { // Don't let bob fall out of the sky
            bob.hitPlatform();
        }
    }
}
 ...
 */
// Update the world with different P2P messages (reliable & unreliable) based on the game P2PMessage message protocol
public void onReceiveTournamentCustomMessage(NextpeerTournamentCustomMessage message) {
	...
	if (msg != null) {
		switch (msg.type) {
		// Updating the opponents height change
		case P2PMessage.Types.P2P_PLAYER_HEIGHT_SO_FAR: {
			P2PMessagePlayerHeight heightMsg = (P2PMessagePlayerHeight)msg;

			OpponentHeightSoFar opponentHeightSoFar = null;
			
			// Do we know that opponent?
			if (playerIdToHeightSoFar.containsKey(message.playerId)) {
				opponentHeightSoFar = playerIdToHeightSoFar.get(message.playerId);
				opponentHeightSoFar.heightSoFar = heightMsg.heightSoFar;
			}
			// Its a new opponent, create its instance
			else {
				opponentHeightSoFar = new OpponentHeightSoFar(message.playerId, message.playerName, heightMsg.heightSoFar);
				playerIdToHeightSoFar.put(message.playerId, opponentHeightSoFar);
			}
		}
			break;
...
private void updateBob(float deltaTime, float accelX) {
	...
	// Create a P2P "height so far" message, encode & send it over unreliable protocol
	P2PMessagePlayerHeight msg = new P2PMessagePlayerHeight(heightSoFar);
    try{
        ...
        Nextpeer.unreliablePushDataToOtherPlayers(msg.encode());
    }
    catch(Exception e) {
    	e.printStackTrace();
    }
...
private void updateCoins(float deltaTime) {
    ...
    private boolean atTheBottomOfScreen() {
    	return (heightSoFar - 7.5f > bob.position.y);
    }
}